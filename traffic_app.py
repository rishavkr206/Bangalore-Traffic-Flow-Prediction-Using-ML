...
class TrafficPredictor:
    ...
    def predict(self, input_data):
        ...
        # Scale input data
        scaled_input = self.scaler.transform(input_data[self.features])
        
        # Create sequence input
        sequence_input = np.zeros((1, self.sequence_length, len(self.features)))
        for i in range(self.sequence_length):
            sequence_input[0, i] = scaled_input[0]
        
        # Make prediction
        prediction = self.model.predict(sequence_input, verbose=0)
        
        # Inverse transform prediction
        prediction_array = np.array([[prediction[0][0]] + [0] * (len(self.features) - 1)])
        original_scale_pred = self.scaler.inverse_transform(prediction_array)[0][0]
    ...
    if uploaded_file is not None:
        try:
            with st.spinner("Loading and processing data..."):
                ...
                st.sidebar.write("Sample of loaded data:")
        ...
        if not st.session_state.training_completed:
            ...
            try:
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    gauge = {
                        'axis': {'range': [None, max(3000, int(prediction) * 1.2)]},
                        'bar': {'color': "darkblue"},
                        'steps': [
